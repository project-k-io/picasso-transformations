...
//AK import android.graphics.Rect;
//AK import android.util.Log;
 ...
 * values are floats.
 */
    public class CropTransformation : Object, Square.Picasso.ITransformation {
        private static  string TAG = "PicassoTransformation";
   ...
   */
        public CropTransformation(int left, int top, int width, int height) {
            ...
            mWidth = width;
            mHeight = height;
            //AK mGravityHorizontal = null;
            //AK mGravityVertical = null;
        }
   ...
   * @param height in pixels
   */
        public CropTransformation(int width, int height): this(width, height, GravityHorizontal.CENTER, GravityVertical.CENTER)
        {
        }
   ...
   *                    100%
   */
        public CropTransformation(float widthRatio, float heightRatio): this(widthRatio, heightRatio, GravityHorizontal.CENTER, GravityVertical.CENTER)
        {
        }
   ...
   */
        ...
        {
            if (Log.IsLoggable(TAG, LogPriority.Verbose)) Log.Verbose(TAG, "transform(): called, " + Key);

            if (mWidth == 0 && mWidthRatio != 0) {
                mWidth = (int) ((float) source.Width * mWidthRatio);
            }
            if (mHeight == 0 && mHeightRatio != 0) {
                mHeight = (int) ((float) source.Height * mHeightRatio);
            }
            ...
            if (mAspectRatio != 0) {
                ...
                if (Log.IsLoggable(TAG, LogPriority.Verbose)) {
                    Log.Verbose(TAG, "transform(): before setting other of h/w: mAspectRatio: " + mAspectRatio
            ...
            if (Log.IsLoggable(TAG, LogPriority.Verbose)) {
                Log.Verbose(TAG, "transform(): created targetRect with width: " + mWidth + ", height: " + mHeight);
            }

            Bitmap bitmap = Bitmap.CreateBitmap(mWidth, mHeight, Bitmap.Config.Argb8888);
            Canvas canvas = new Canvas(bitmap);
            if (Log.IsLoggable(TAG, LogPriority.Verbose)) {
                Log.Verbose(TAG, "transform(): copying from source with width: " + source.Width + ", height: "
                           + source.Height);
            }
            ...
            if (Log.IsLoggable(TAG, LogPriority.Verbose)) {
                Log.Verbose(TAG, "transform(): returning bitmap with width: " + bitmap.Width + ", height: "
                           + bitmap.Height);
            }
        ...
        private int getTop(Bitmap source) {
            switch (mGravityVertical) {
                case GravityVertical.TOP:
                    return 0;
                case GravityVertical.CENTER:
                    return (source.Height - mHeight) / 2;
                case GravityVertical.BOTTOM:
                    return source.Height - mHeight;
                default:
                    return 0;
            }
        }
        ...
        private int getLeft(Bitmap source) {
            switch (mGravityHorizontal) {
                case GravityHorizontal.LEFT:
                    return 0;
                case GravityHorizontal.CENTER:
                    return (source.Width - mWidth) / 2;
                case GravityHorizontal.RIGHT:
                    return source.Width - mWidth;
                default:
                    return 0;
            }
        }
        ...
        public string Key =>  "CropTransformation(width=" + mWidth + ", height=" + mHeight + ", mWidthRatio=" + mWidthRatio + ", mHeightRatio=" + mHeightRatio + ", mAspectRatio=" + mAspectRatio + ", gravityHorizontal=" + mGravityHorizontal + ", mGravityVertical=" + mGravityVertical + ")";
